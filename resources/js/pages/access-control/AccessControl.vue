    ...
    <AppLayout title="Access Matrix">
        <div class="bg-gray-50 dark:bg-gray-950 min-h-screen py-8 px-4">
            <div class="max-w-7xl mx-auto">
                ...
                <div class="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center px-2 gap-2">
                ...
                <div class="hidden lg:block bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-xl overflow-hidden transition-all">
                ...
                <div class="lg:hidden space-y-4">
                    <div v-for="mod in modules" :key="mod.slug" class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 shadow-sm overflow-hidden">
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="font-black text-gray-900 dark:text-white uppercase text-xs tracking-wider">{{ mod.name }}</h3>
                                <p class="text-[9px] text-gray-400 font-mono italic">{{ mod.slug }}.*</p>
                            </div>
                            <button type="button" @click="toggleRow(mod.slug)"
                                    class="px-4 py-2 text-[9px] font-black uppercase rounded-lg border transition-all"
                                    :class="isRowFull(mod.slug) ? 'bg-blue-600 border-blue-600 text-white' : 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-400'">
                                {{ isRowFull(mod.slug) ? 'Revoke All' : 'Select All' }}
                            </button>
                        </div>

                        <div class="grid grid-cols-2 gap-3">
                            <button v-for="action in actions" :key="action"
                                    @click="togglePermission(mod.slug, action)"
                                    class="flex items-center gap-3 p-3 rounded-xl border-2 transition-all text-left"
                                    :class="hasPermission(mod.slug, action)
                                ...
                                <div class="size-5 shrink-0 rounded-md flex items-center justify-center border"
                                     :class="hasPermission(mod.slug, action) ? 'bg-blue-600 border-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 border-gray-200 dark:border-gray-600'">
                                    <Check v-if="hasPermission(mod.slug, action)" class="w-3 h-3 stroke-[4]" />
                                    <Plus v-else class="w-3 h-3 opacity-20" />
                                </div>
                                <span class="text-[10px] font-black uppercase tracking-widest truncate">{{ action }}</span>
                            </button>
                        </div>
                    </div>
                </div>

                <div class="mt-8 p-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl flex flex-col sm:flex-row justify-end items-center gap-4">
                    <Link href="/dashboard" class="px-6 py-3 text-xs font-bold text-gray-400 hover:text-gray-600">Discard Changes</Link>
                    <button @click="savePermissions" :disabled="form.processing"
                            class="w-full sm:w-auto px-12 py-4 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-blue-500/20 hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2">
                        <Save class="w-4 h-4" />
