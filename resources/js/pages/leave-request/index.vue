            ...
            <div class="flex-1 overflow-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                    ...
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase">AL</th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase">CL</th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase">SL</th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase">MAT</th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase">PAT</th>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase">Total</th>
                    ...
                    <tr v-for="(l, i) in leaveRequests.data" :key="l.id" class="hover:bg-gray-100 dark:hover:bg-gray-800">
                        <td class="px-4 py-2 text-sm">{{ (leaveRequests.current_page - 1) * leaveRequests.per_page + i + 1 }}</td>
                        <td class="px-4 py-2 text-sm">{{ l.person_name }}</td>
                ...
                >
                  {{ l.status === 0 ? 'Pending' : l.status === 2 ? 'Approved' : 'Rejected' }}
                    ...
                    <tr v-if="!leaveRequests.data.length">
                        <td colspan="14" class="text-center py-4 text-gray-500 dark:text-gray-400">No leave requests found.</td>
                    </tr>
                    </tbody>
                </table>
            </div>
