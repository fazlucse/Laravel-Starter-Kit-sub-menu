      ...
      >
        ...
        <div class="space-y-3">
          ...
          <input
            ...
            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
          />
            <div class="space-y-1">
                <select
                    v-model="filters.department_id"
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer"
                >
            ...
            <div class="space-y-1">
                <select
                    ...
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer"
                >
                    <option value="" class="text-gray-900 dark:text-white">Select Division</option>
                    <option v-for="opt in divisionOptions" :key="opt.id" :value="opt.id">{{ opt.name }}</option>
                </select>
            </div>
...
const props = defineProps<{
  ...
  preserve?: Record<string, any>,
    divisionOptions: Array<{ id: number | string, name: string }>
    departmentOptions: Array<{ id: number | string, name: string }> // Added department options
}>()
...
watch(
  () => props.modelValue,
  (v) => {
    if (v && !filters.value.employee_name) {
      filters.value.employee_name = v
    }
  }
)
